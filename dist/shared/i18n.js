...
export function normalizeSiteLocale(locale) {
...
function toJavaScriptWeekday(day) {
    if (day === 7) {
        return 0;
    }
    if (day >= 0 && day <= 6) {
...
export function deriveWeekFirstDay(locale) {
    const normalized = normalizeSiteLocale(locale);
    try {
...
export function buildWeekdayLabels(locale, weekFirstDay) {
